...
const materialFileAccess = windows.FILE_READ_DATA |
...
// whose material allow entries name only that owner, LocalSystem, or Builtin Administrators.
func ValidatePrivateFile(path string) error {
	...
	allowed := []*windows.SID{ownerSID, systemSID, administratorsSID}
	for index := uint32(0); index < uint32(dacl.AceCount); index++ {
		...
		if ace.Header.AceType != windows.ACCESS_ALLOWED_ACE_TYPE {
			return fmt.Errorf("%w: unsupported allow entry", ErrInsecureFile)
		}
		if ace.Mask&materialFileAccess == 0 {
			continue
		}
		sid := (*windows.SID)(unsafe.Pointer(&ace.SidStart))
		if !sidAllowed(sid, allowed) {
			return fmt.Errorf("%w: material access granted to unexpected principal", ErrInsecureFile)
		}
	}
	return nil
}

// ProtectPrivateFile replaces path's DACL with protected full-control entries for the
// current owner, LocalSystem, and Builtin Administrators.
func ProtectPrivateFile(path string) error {
	...
	sids := []*windows.SID{ownerSID, systemSID, administratorsSID}
